...
export async function downloadYouTubeAudio(url: string, outputDir: string): Promise<{ audioPath: string, videoId: string }> {
  ...
  try {
    await execPromise(`yt-dlp -x --audio-format mp3 -o "${audioPath}" "${cleanUrl}"`);
  } catch (error: any) {
...
export async function getVideoMetadata(url: string) {
  const cleanUrl = cleanYouTubeUrl(url);
  try {
    const { stdout } = await execPromise(`yt-dlp --dump-json "${cleanUrl}"`);
...
export async function getYouTubeTranscript(url: string, outputDir: string): Promise<TranscriptSegment[]> {
  ...
  try {
    ...
    await execPromise(
      `yt-dlp --write-auto-sub --write-sub --sub-lang "en.*,en" --sub-format json3 --skip-download -o "${subtitlePath}" "${cleanUrl}"`,
